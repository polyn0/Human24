...
from transformers_mrc.data import glue_compute_metrics as compute_metrics
from transformers_mrc.data import glue_output_modes as output_modes
from transformers_mrc.data import glue_processors as processors
from transformers_mrc.data import glue_convert_examples_to_features as convert_examples_to_features
import sys
import csv
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        epoch_iterator = tqdm(train_dataloader,total=len(train_dataloader), desc="Iteration", disable=args.local_rank not in [-1, 0])
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, predict=False):
    ...
    if predict:
        ...
        examples = processor.get_dev_examples(args.data_dir) if evaluate else processor.get_train_examples(
            args.data_dir)
    features, id_map = convert_examples_to_features(examples, tokenizer, label_list=label_list,
                                                    max_length=args.max_seq_length, output_mode=output_mode,
                                                    pad_on_left=False,
                                                    pad_token=tokenizer.convert_tokens_to_ids([tokenizer.pad_token])[0],
                                                    pad_token_segment_id=0, output_feature=True, )
    # if args.local_rank in [-1, 0]:
